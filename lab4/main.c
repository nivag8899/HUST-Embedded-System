...
static int radius[5] = {40,40,40,40,40};
...
static void touch_event_cb(int fd)
{
	...
	case TOUCH_PRESS:
		...
		// fb_draw_rect(x - 40, y - 40, 80, 80, color);
		fb_draw_text(x - 50, y + 25, "●", 100, color);
		old[finger].x = x;
		old[finger].y = y;
		break;
	case TOUCH_MOVE:
		...
		fb_draw_rect(old[finger].x - 50, old[finger].y - 50, 100, 100, COLOR_BACKGROUND);
		// fb_draw_circle(x, y, radius[finger], color);

		// fb_draw_rect(x - 40, y - 40, 85, 85, color);
		fb_draw_text(x - 50, y + 25, "●", 100, color);

		// fb_draw_pixel(x, y + 10, BLACK);
		old[finger].x = x;
		old[finger].y = y;
		break;
	case TOUCH_RELEASE:
